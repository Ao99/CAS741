...
initial \wss{Does the word ``initial'' have significance here?  I think you can
  just say ``input data''.}  input data. \progname{}
...
final \wss{As above, do you need the word ``final''?} output data.

\item[\refstepcounter{acnum} \actheacnum
  \label{acCalculation}:]\progname{} only uses global threshold with Otsu's
  Method. \wss{If you mention Otsu's method, then you should give a citation to
    where this method is introduced/explained.  I also wonder what is the secret
    here.  Is it a secret the Otsu's method is being used, or will the interface
...
processed image and segmentation image, it can make more thoroughly verificationin the future.
...
now. \progname{}
manipulates multi-frame images with a sequence of image data created from it. Itmay use other image data structures in the future, such as a 3D sequence
...
;
\end{tikzpicture}
\captionof{figure}{Major functions of \famname{} \cite{Dong2019ca}}
...
\item[\refstepcounter{ucnum} \uctheucnum \label{uc_Input}:] 
There will always be a source of input data external to the software.

\item[\refstepcounter{ucnum} \uctheucnum \label{uc_Goal}:] 
